...
module.exports = async (req, res) => {
  ...
  const pResult = await db.execute({ sql: 'SELECT id, gender FROM participants WHERE id = ?', args: [participant_id] });
  if (pResult.rows.length === 0) return res.status(400).json({ error: '参加者不存在' });
  const p = pResult.rows[0];

  const existing = await db.execute({ sql: 'SELECT id FROM submissions WHERE participant_id = ?', args: [participant_id] });
  ...
  for (const pick of picks) {
    const picked = await db.execute({ sql: 'SELECT id, gender FROM participants WHERE id = ?', args: [pick.id] });
  ...
  await db.execute({
    sql: 'INSERT INTO submissions (participant_id, pick1_id, pick2_id, pick3_id) VALUES (?, ?, ?, ?)',
    args: [participant_id, padded[0].id, padded[1].id, padded[2].id]
  });
